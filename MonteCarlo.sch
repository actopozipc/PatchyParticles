...
def berechne_totale_energie(teilchen_liste):
    ...
    für i in reichweite(anz_teilchen):
        für j in reichweite(i + 1, anz_teilchen):
            ...
            #otherwise calculate the distance between the points and not between the center of the particles
            distanz = teilchen_liste[i].calculate_distance(teilchen_liste[j])
            energie_zwischen_zwei_teilchen = Potentials.interaction_potential(teilchen_liste[i].top_patch, teilchen_liste[j].top_patch, distanz) +  Potentials.interaction_potential(teilchen_liste[i].bottom_patch, teilchen_liste[j].bottom_patch, distanz)
